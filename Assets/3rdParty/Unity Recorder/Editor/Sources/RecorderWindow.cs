    ...
    public class RecorderWindow : EditorWindow
    {
        ...
        string CheckRecordersIncompatibility()
        {
            ...
            foreach (var recorder in activeRecorders)
            {
                ...
                if (gameView != null)
                {
                    if (gameViewRecorders.Any() && !gameViewRecorders.ContainsKey(gameView.outputHeight))
                    {
                    ...
                    gameViewRecorders[gameView.outputHeight] = recorder;
                }
            }
        ...
        {        
            ...
            m_RecorderController.verbose = Options.verboseMode;
        ...
        void OnRecorderSettingPresetClicked()
        {
            if (m_SelectedRecorderItem != null && m_SelectedRecorderItem.settings != null)
            {
                ...
                presetReceiver.Init(m_SelectedRecorderItem.settings, Repaint);
        ...
        static void DisplayStatusBarMessage(string msg, MessageType messageType)
        {
        ...
        void UpdateRecordingProgressGUI()
        {
            ...
            {                
                if (!HaveActiveRecordings())
                {
                    DisplayStatusBarMessage("No active recorder", MessageType.Info);
                }
                else
                {
                    ...
                    {
                        DisplayStatusBarMessage("Ready to start recording", MessageType.None);
                    }
                    else
                    {
                        DisplayStatusBarMessage(msg, MessageType.Warning);
                    }
                }
