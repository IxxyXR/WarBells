...
namespace UnityEditor.Recorder
{
    static class EnumHelper
    {
        ...
        internal static int GetEnumValueFromClippedIndex<TEnum>(int index, int start, int end)
        {
            if (!typeof(TEnum).IsEnum) throw new ArgumentException("Arg not an enum");
            var values = Enum.GetValues(typeof(TEnum));
            for( int i = 0, j = -1; i < values.Length; i++ )
        ...
        internal static int GetClippedIndexFromEnumValue<TEnum>(int value, int start, int end)
        {
            if (!typeof(TEnum).IsEnum) throw new ArgumentException("Arg not an enum");
            var values = Enum.GetValues(typeof(TEnum));
            for( int i = 0, j = -1; i < values.Length; i++ )
        ...
        internal delegate string EnumToStringDelegate<in TEnum>(TEnum value); 
